...
# bledna 
def calculate_bond_pv(coupon, discount_factor, bond_yield, face_value):
    pv = coupon * ((1 - discount_factor) / bond_yield) + (face_value * discount_factor)
    return pv
...
def update_market_state(tick):
    global events_counter, last_market_event_time
    
    logging.info(f"Received tick for: {tick.get('symbol')}")
    
    with state_lock:
        events_counter += 1
        last_market_event_time = tick.get("timestamp")
...
def recalculate_valuations(tick):
    ...
    if asset_type == "EQUITY":
        ...
        if bond_yield is not None:
            ...
            maturity_years = 5
            discount_factor = (1 + bond_yield) ** -maturity_years
            pv = round(calculate_bond_pv(coupon, discount_factor, bond_yield, face_value), 4)
