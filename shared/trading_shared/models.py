...
class Trade(Base):
    ...
    metadata_payload = Column('metadata', JSONB, nullable=True) # Słowo 'metadata' jest zarezerwowane w SQLAlchemy, więc używamy aliasu
    created_at = Column(DateTime(timezone=True), nullable=False, default=utc_now)
    updated_at = Column(DateTime(timezone=True), nullable=False, default=utc_now, onupdate=utc_now)
...
class MarketDataCurve(Base):
    ...
    event_id = Column(BigInteger, nullable=True)
    curve_name = Column(Text, nullable=False) # YIELD_CURVE / FX_FORWARD_CURVE / DISCOUNT_CURVE
...
class AuditLog(Base):
    ...
    event_type = Column(Text, nullable=False) # CREATED / UPDATED / DELETED / ERROR / STREAM_CONNECTED etc.
    entity_type = Column(Text, nullable=True) # TRADE / BOOK / VALUATION / MARKET_DATA etc.
